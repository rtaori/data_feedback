...
class ResNet18Network(nn.Module):
    ...
    def forward(self, batch):
        batch['classifier'] = self.net(batch['input'].float())
        batch['loss'] = self.criterion(batch['classifier'], batch['target'])
        batch['correct'] = batch['classifier'].max(dim = 1).indices == batch['target']
        return batch


class ResNetOptimizer():
    def __init__(self, model, lr, T_max):
        self.optimizer = torch.optim.SGD(model.parameters(), lr=lr, momentum=0.9, weight_decay=5e-4)
